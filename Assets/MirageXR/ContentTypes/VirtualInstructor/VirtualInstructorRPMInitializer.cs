...
namespace MirageXR
{
    public class VirtualInstructorRPMInitializer : AvatarInitializer
    {
        ...
        public async override UniTask InitializeAvatarAsync(GameObject avatar)
        {
            ...
            Transform headTopEnd = avatar.transform.Find("Armature/Hips/Spine/Spine1/Spine2/Neck/Head/HeadTop_End");
            Vector3 height = headTopEnd.position - avatar.transform.position;
            bool isMale = height.sqrMagnitude > 3.2f;
            string avatarDefinitionPath = isMale ? "AnimationAvatars/Masculine" : "AnimationAvatars/Feminine";
            Debug.Log($"Applying {(isMale ? "male" : "female")} avatar to {avatar.name}");

            GameObject template = Resources.Load<GameObject>(avatarDefinitionPath);
