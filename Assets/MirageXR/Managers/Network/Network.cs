...
namespace MirageXR.NewDataModel
{
    public static class Network
    {
        ...
        public static async UniTask<Response> RequestAsync<T>(Stream stream, string url, HttpContent content = null, string token = null, Dictionary<string, string> header = null, RequestType type = RequestType.Get, float timeout = Timeout, CancellationToken cancellationToken = default)
        {
            HttpResponseMessage response = null;
            try
            {
        ...
        private static FailedResponseData HandleWebException(in WebException e)
        {
            ...
            switch (e.Status)
            {
                case WebExceptionStatus.Success: // 0
                    error = $"No error was encountered.{responseString}";
                    return new FailedResponseData(ResponseStatusCode.OK, error);
                case WebExceptionStatus.NameResolutionFailure: // 1
                    error = $"The name resolver service could not resolve the host name.{responseString}";
                    return new FailedResponseData(ResponseStatusCode.NoConnection, error);
                case WebExceptionStatus.ConnectFailure: // 2
                    error = $"The remote service could not be contacted at the transport level.{responseString}";
                    return new FailedResponseData(ResponseStatusCode.NoConnection, error);
                case WebExceptionStatus.ReceiveFailure: // 3
                    error = $"A complete response was not received from the remote server.{responseString}";
                    return new FailedResponseData(ResponseStatusCode.NoConnection, error);
                case WebExceptionStatus.SendFailure: // 4
                    error = $"A complete request could not be sent to the remote server.{responseString}";
                    return new FailedResponseData(ResponseStatusCode.NoConnection, error);
                case WebExceptionStatus.PipelineFailure: // 5
                    error = $"The connection was closed before the response for a pipelined request was received.{responseString}";
                    return new FailedResponseData(ResponseStatusCode.NoConnection, error);
                case WebExceptionStatus.RequestCanceled: // 6
                    error = $"The request was canceled / aborted.{responseString}";
                    return new FailedResponseData(ResponseStatusCode.RequestTimeout, error);
                case WebExceptionStatus.ProtocolError: // 7
                    error = $"The response received from the server indicated a protocol-level error.{responseString}";
                    return new FailedResponseData(ResponseStatusCode.ProtocolError, error);
                case WebExceptionStatus.ConnectionClosed: // 8
                    error = $"The connection was prematurely closed.{responseString}";
                    return new FailedResponseData(ResponseStatusCode.NoConnection, error);
                case WebExceptionStatus.TrustFailure: // 9
                    error = $"A server certificate could not be validated.{responseString}";
                    return new FailedResponseData(ResponseStatusCode.NoConnection, error);
                case WebExceptionStatus.SecureChannelFailure: // 10
                    error = $"An error occurred while establishing a connection using SSL.{responseString}";
                    return new FailedResponseData(ResponseStatusCode.NoConnection, error);
                case WebExceptionStatus.ServerProtocolViolation: // 11
                    error = $"The server response was not a valid HTTP response.{responseString}";
                    return new FailedResponseData(ResponseStatusCode.NoConnection, error);
                case WebExceptionStatus.KeepAliveFailure: // 12
                    error = $"The connection for a request that specifies the Keep-alive header was closed unexpectedly.{responseString}";
                    return new FailedResponseData(ResponseStatusCode.NoConnection, error);
                case WebExceptionStatus.Pending: // 13
                    error = $"An internal asynchronous request is pending.{responseString}";
                    return new FailedResponseData(ResponseStatusCode.ProtocolError, error);
                case WebExceptionStatus.Timeout: // 14
                    error = $"No response was received during the time-out period for a request.{responseString}";
                    return new FailedResponseData(ResponseStatusCode.Timeout, error);
                case WebExceptionStatus.ProxyNameResolutionFailure: //15
                    error = $"The name resolver service could not resolve the proxy host name.{responseString}";
                    return new FailedResponseData(ResponseStatusCode.NoConnection, error);
                case WebExceptionStatus.UnknownError: // 16
                    error = $"An exception of unknown type has occurred.{responseString}";
                ...
                case WebExceptionStatus.MessageLengthLimitExceeded: // 17
                    error = $"A message was received that exceeded the specified length limit.{responseString}";
                ...
                case WebExceptionStatus.CacheEntryNotFound: // 18
                    error = $"The specified cache entry was not found.{responseString}";
                    return new FailedResponseData(ResponseStatusCode.NotFound, error);
                case WebExceptionStatus.RequestProhibitedByCachePolicy: // 19
                    error = $"The request was not permitted by the cache policy.{responseString}";
                    return new FailedResponseData(ResponseStatusCode.ProtocolError, error);
                case WebExceptionStatus.RequestProhibitedByProxy: // 20
                    error = $"This request was not permitted by the proxy.{responseString}";
                    return new FailedResponseData(ResponseStatusCode.ProtocolError, error);
                default:
                    error = $"Unknown WebExceptionStatus={e.Status}.{responseString}";
