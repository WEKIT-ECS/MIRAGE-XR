...
public class ColorPicker : UIComponent {
  ...
  private void Update(){
    if(CoreServices.FocusProvider.PrimaryPointer != null && CoreServices.FocusProvider.PrimaryPointer.Rays.Length > 0)
      UpdateColorSelectorAndSlider(CoreServices.FocusProvider.PrimaryPointer.Rays[0]);
  }

  void UpdateColorSelectorAndSlider(Ray inputRay) {
    ...
    RaycastHit hitInfo;
    if (CoreServices.FocusProvider.PrimaryPointer.Result.CurrentPointerTarget == m_ColorPickerSlider.gameObject) {
      ...
      if (!validCollision && m_ActiveInputObject == m_ColorPickerSlider.gameObject) {
        validCollision = BasePanel.DoesRayHitCollider(inputRay, m_ColorPickerSlider.GetCollider(), out hitInfo);
      }

      if (validCollision) {
        // Over slider, check for mouse down.
        if (m_ColorPickerSlider.isSelected) {
    ...
    if (CoreServices.FocusProvider.PrimaryPointer.Result.CurrentPointerTarget == m_ColorPickerSelector.gameObject) {
      Debug.Log(CoreServices.FocusProvider.PrimaryPointer.Result.CurrentPointerTarget);
      if (BasePanel.DoesRayHitCollider(inputRay, m_ColorPickerSelector.GetCollider(),
          out hitInfo)) {
        // Over color picker, check for input.
        if (m_ColorPickerSelector.isSelected) {
