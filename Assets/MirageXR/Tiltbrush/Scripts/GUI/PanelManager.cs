...
namespace TiltBrush {
    ...
    public bool IsValidForSdkMode(SdkMode mode) {
      switch (mode) {
        ...
        case SdkMode.Monoscopic:
          return m_ModeMono;
        case SdkMode.Mrtk:
          //We should check if trackedHands mode. Mrtk can be used without hand tracking
          return m_TrackedHands;
        default:
  ...
  public class PanelManager : MonoBehaviour {
    ...
    void CreatePanel(PanelMapKey key, bool advancedPanel) {
      if(key.m_PanelPrefab == null)
        return;
        
      GameObject obj = Instantiate(key.m_PanelPrefab, transform);
    ...
    void OnDestroy(){
      Destroy(m_WandPaneVisuals.gameObject);
    }
  }
