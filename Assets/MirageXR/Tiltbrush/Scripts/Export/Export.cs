...
URL=" + kExportDocumentationUrl;
  ...
  public static void ExportScene(string directory) {
    ...
    string parent = FileUtils.GenerateNonexistentFilename(directory, basename, "");
    if (!FileUtils.InitializeDirectoryWithUserError(
    ...
    ControllerConsoleScript.m_Instance.AddNewLine("Located in " + directory);

    string readmeFilename = Path.Combine(directory, kExportReadmeName);
