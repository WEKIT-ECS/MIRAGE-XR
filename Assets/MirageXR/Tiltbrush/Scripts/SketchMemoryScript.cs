...
namespace TiltBrush
{
    ...
    public class SketchMemoryScript : MonoBehaviour
    {
        ...
        bool playbackPaused;
        double pausedSketchTime;

        public void SetPlaybackPause(bool pause)
        {
            if (pause == playbackPaused)
                return;

            playbackPaused = pause;
            if (playbackPaused)
                pausedSketchTime = App.Instance.CurrentSketchTime;
            else
                App.Instance.CurrentSketchTime = pausedSketchTime;
        }

        public bool IsPlaying => m_ScenePlayback != null && !playbackPaused;
        public bool IsDonePlaying => m_ScenePlayback == null;
        ...
        public void EndDrawingFromMemory()
        {
            if(m_ScenePlayback == null)
              return;
    ...
    {
        Destroy(m_UndoBatchMesh.gameObject);
    }
}
} // namespace TiltBrush
