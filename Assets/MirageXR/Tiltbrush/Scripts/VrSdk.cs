...
namespace TiltBrush {
  ...
  [SerializeField] private SteamVR_Overlay m_SteamVROverlay;
    ...
    [SerializeField] private GameObject m_HandsControlsPrefab;
  ...
  private Compositor_FrameTiming m_FrameTiming;
    ...
    public void Init(GameObject vrSystem, Camera vrCamera) {
      m_VrSystem = vrSystem;
      m_VrCamera = vrCamera;
      ...
      } else if (App.Config.m_SdkMode == SdkMode.Mrtk) {

        SetControllerStyle(ControllerStyle.Hands);
        //TODO-SAID: Configure the rest ?
    ...
    void Start() {
      if (App.Config.m_SdkMode == SdkMode.SteamVR) {
        ...
        MRTKHandTrackingManager.NewPosesApplied += OnNewPoses;

        MRTKHandTrackingManager.SourceDetected += (eventData) => {
          if (eventData.InputSource.SourceType == InputSourceType.Head)
            OnInputFocus(true);
        };

        MRTKHandTrackingManager.SourceLost += (eventData) => {
          if (eventData.InputSource.SourceType == InputSourceType.Head)
            OnInputFocus(false);
          // else if (eventData.InputSource.SourceType == InputSourceType.Hand)
        };
      }

      if (ControllerConsoleScript.m_Instance != null && m_NeedsToAttachConsoleScript && m_VrControls != null) {
  ...
  static private Vector3 UnityFromSteamVr(HmdVector3_t v) {
    ...
    public void SetControllerStyle(ControllerStyle style) {
      ...
      switch (style) {
        ...
        case ControllerStyle.Hands:
          controlsPrefab = m_HandsControlsPrefab;
          break;
    ...
    public ControllerInfo CreateControllerInfo(BaseControllerBehavior behavior, bool isLeftHand) {
      if (App.Config.m_SdkMode == SdkMode.SteamVR) {
        ...
        //TODO-SAID: Make dedicated controller info ?
        return new TrackedHandControllerInfo(behavior);
      } else {
